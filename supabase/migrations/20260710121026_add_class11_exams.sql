...
DECLARE
  ...
  v_valid_standards TEXT[] := ARRAY['XI Science'];
BEGIN
  ...
  CREATE TEMP TABLE temp_exams (name TEXT, term TEXT);
  INSERT INTO temp_exams (name, term) VALUES 
    ('CT-1', 'Term-1'),
    ('CT-2', 'Term-1'),
    ('Term-1', 'Term-1'),
    ('CT-3', 'Term-2'),
  ...
  FOR v_std IN SELECT id, name FROM public.standards WHERE name = ANY(v_valid_standards) LOOP
      ...
      FOR v_exam IN SELECT name, term FROM temp_exams LOOP
        ...
        FOR v_sub IN SELECT id, name, evaluation_type FROM public.subjects WHERE standard_id = v_std.id LOOP
          ...
          IF NOT EXISTS (SELECT 1 FROM public.exam_subjects WHERE exam_id = v_exam_id AND subject_id = v_sub.id) THEN
            ...
            IF v_sub.evaluation_type = 'mark' THEN
              IF v_exam.name IN ('Term-1', 'Term-2') THEN
                IF v_sub.name IN ('Physics', 'Chemistry', 'Biology', 'Physical Education') THEN
                  v_final_max := 70;
                ELSE
                  v_final_max := 80;
                END IF;
            ...
            VALUES (
              ...
              v_sub.id, 
              v_final_max,
              CASE WHEN v_sub.evaluation_type = 'mark' THEN CEIL(v_final_max * 0.33) ELSE 33 END,
              v_user_id, 
              v_user_id
            );
          END IF;
